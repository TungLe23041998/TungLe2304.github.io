...
class Background extends Entity {constructor(...args) {super(...args);_defineProperty(this, "draw",
    ...
    });}drawText({ ctx, canvas1 }) {const ms = Math.min(canvas1.width, canvas1.height);const size = ms / 15;const copy = 'Waves';const x = canvas1.width / 2;const y = canvas1.height / 3 + size / 3;ctx.font = `700 italic ${size}px futura, sans-serif`;ctx.textAlign = 'center';ctx.fillStyle = '#3C9E9A';ctx.fillText(copy, x, y);}drawGradient({ ctx, canvas1, bounds }) {// const gradient = ctx.createLinearGradient(...bounds.params);
    // gradient.addColorStop(0, '#333');
    // gradient.addColorStop(1, '#222');
    // ctx.fillStyle = gradient;
    ctx.fillStyle = '#091619'; // ctx.globalAlpha = 0.9;
    ctx.fillRect(...bounds.params); // ctx.globalAlpha = 1;
  }} //*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡/
// Canvas
//*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡*/
class Canvas1 {constructor({ canvas1, entities = [], pointer }) {_defineProperty(this, "setCanvasSize",
    ...
    () => {
      ...
      const w2 = w * this.dpr;
      const h2 = h * this.dpr;
      this.canvas1.width = w2;
      this.canvas1.height = h2;
      this.canvas1.style.width = w + 'px';
      this.canvas1.style.height = h + 'px';
    ...
    this.canvas1 = canvas1;this.dpr = window.devicePixelRatio || 1;this.ctx = canvas1.getContext('2d');this.ctx.scale(this.dpr, this.dpr); // tick counter
...
class PolyWave extends Entity {
  constructor({ verts, color, elasticity, damping }) {
    ...
    this.color = color;this.points = [];this.resolution = 50;this.elasticity = elasticity;this.damping = damping;this.constructPolyWave();this.setAttractors();}constructPolyWave() {for (let i = 0; i < this.verts.length; i++) {const p1 = this.verts[i];const p2 = this.verts[i + 1];if (p1 && p2) {const [dx, dy] = p2.point.delta(p1.point);const distance = p2.point.distance(p1.point);const amount = distance / this.resolution;const pointAmt = Math.round(amount);const offX = dx / pointAmt;const offY = dy / pointAmt;if (p1.isSpring) {for (let k = 1; k <= pointAmt; k++) {// debugger;
            const x = p1.point.x + offX * k;const y = p1.point.y + offY * k;const point = new Spring({ x, y, elasticity: this.elasticity, damping: this.damping, isFixed: k === 0 || k === pointAmt });this.points.push(point);}} else {this.points.push(new Spring({ x: p2.point.x, y: p2.point.y, isFixed: true }));}}}}setAttractors() {this.points.forEach((p, i) => {const isLast = i === this.points.length - 1;const isFirst = i === 0;if (isLast) {const prevPoint = this.points[i - 1];const nextPoint = this.points[0];!p.isFixed && p.addAttractor(prevPoint);!p.isFixed && p.addAttractor(nextPoint);} else if (isFirst) {const prevPoint = this.points[this.points.length - 1];const nextPoint = this.points[i + 1];!p.isFixed && p.addAttractor(prevPoint);!p.isFixed && p.addAttractor(nextPoint);} else {const prevPoint = this.points[i - 1];const nextPoint = this.points[i + 1];!p.isFixed && p.addAttractor(prevPoint);!p.isFixed && p.addAttractor(nextPoint);}});}}
//*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡/
// Spring
//*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡*/
...
class Spring extends Point {
  ...
  addAttractor(point) {this.attractors = [...this.attractors, point];}setAdjacentForces() {// currently unused, was testing out an
    ...
    this.fx = 0;this.fy = 0;}}const MOUSE_STRENGTH = 1; // 0 - 1
const MOUSE_RADIUS = 200 * DPR;const colors = ['#8ac8bb', '#3e7372', '#7bc4a2', '#335b67', '#1d4052', '#152b35'];const center = new Point(window.innerWidth / 2 * DPR, window.innerHeight / 2 * DPR);
